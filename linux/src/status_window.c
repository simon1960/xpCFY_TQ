...
static void draw_status_window(XPLMWindowID window, void* refcon)
{
	...
	draw_text("Connection state:", left + 24, top - 48, status.connected ? connected_colour : disconnected_colour);
	draw_text(status.connected ? "CONNECTED" : "DISCONNECTED", left + 169, top - 48, status.connected ? connected_colour : disconnected_colour);
...
static int handle_mouse_wheel(XPLMWindowID window, int x, int y, int wheel, int clicks, void* refcon)
{
	(void)window;
	(void)x;
	(void)y;
