...
void plugin_config_defaults(PluginConfig* config)
{
	...
	/* A genuinely new configuration starts with diagnostic tracing disabled. */
	config->enhanced_logging = 0;
}
 ...
 */
static int ini_network_protocol(const char* path, int fallback, int* needs_rewrite)
{
	char value[16];
	DWORD length;

	*needs_rewrite = 0;
	length = GetPrivateProfileStringA("connection", "network_protocol", "", value, (DWORD)sizeof(value), path);
	if (length != 0)
	{
		if (_stricmp(value, "UDP") == 0)
			return (1);
		if (_stricmp(value, "TCP") == 0)
			return (0);
		log_write("Invalid network_protocol value '%s'; using %s", value, fallback ? "UDP" : "TCP");
		*needs_rewrite = 1;
		return (fallback);
	}

	length = GetPrivateProfileStringA("connection", "network_use_udp", "", value, (DWORD)sizeof(value), path);
	*needs_rewrite = 1;
	if (length != 0)
	{
		if (strcmp(value, "1") == 0)
		...
		if (strcmp(value, "0") == 0)
			return (0);
		log_write("Invalid legacy network_use_udp value '%s'; using %s", value, fallback ? "UDP" : "TCP");
	}
	return (fallback);
}
...
{
	char path[MAX_PATH];
	char enhanced_logging[16];
	DWORD attributes;
	DWORD enhanced_logging_length;
	int needs_protocol_rewrite;
	int needs_enhanced_logging_rewrite;
	...
	config->network_use_udp = ini_network_protocol(path, config->network_use_udp, &needs_protocol_rewrite);
	config->require_cfy_user_id = ini_uint(path, "require_cfy_user_id", config->require_cfy_user_id, 0, 1) != 0;
	enhanced_logging_length = GetPrivateProfileStringA("connection", "enhanced_logging", "", enhanced_logging, (DWORD)sizeof(enhanced_logging), path);
	needs_enhanced_logging_rewrite = enhanced_logging_length == 0;
	config->enhanced_logging = needs_enhanced_logging_rewrite ? 0 : ini_uint(path, "enhanced_logging", config->enhanced_logging, 0, 1) != 0;
	log_write("Configuration loaded from %s (TQ variant %s, PoKeys network protocol %s, enhanced logging %s)", path, config->trim_motor_variant == 3U ? "V3" : (config->trim_motor_variant == 5U ? "Pro" : "V4"), config->network_use_udp ? "UDP" : "TCP", config->enhanced_logging ? "enabled" : "disabled");
	if (needs_protocol_rewrite || needs_enhanced_logging_rewrite)
	{
		log_write("Migrating plugin configuration to persist %s%s%s", needs_protocol_rewrite ? "the selected PoKeys protocol" : "", needs_protocol_rewrite && needs_enhanced_logging_rewrite ? " and " : "", needs_enhanced_logging_rewrite ? "enhanced logging disabled for this existing file" : "");
		if (!plugin_config_write(config))
			return (0);
	}
	return (1);
}
...
int plugin_config_write(const PluginConfig* config)
{
	...
	UINT persisted_enhanced_logging;
	...
	fprintf(stream, "enhanced_logging=%d\n", config->enhanced_logging ? 1 : 0);
	...
	if (ok)
	{
		...
		GetPrivateProfileStringA("connection", "network_protocol", "", persisted_protocol, (DWORD)sizeof(persisted_protocol), path);
		persisted_variant = GetPrivateProfileIntA("connection", "trim_motor_variant", 0, path);
		persisted_enhanced_logging = GetPrivateProfileIntA("connection", "enhanced_logging", 2, path);
		ok = _stricmp(persisted_protocol, config->network_use_udp ? "UDP" : "TCP") == 0 && persisted_variant == config->trim_motor_variant && persisted_enhanced_logging == (UINT)(config->enhanced_logging ? 1 : 0);
		if (!ok)
