...
void log_write_at(const char* source_file, unsigned int source_line, const char* format, ...)
{
	...
	{
		GetLocalTime(&now);
		fprintf(stream, "%02u.%02u.%04u %02u:%02u:%02u.%03u %30.30s %5u ", now.wDay, now.wMonth, now.wYear, now.wHour, now.wMinute, now.wSecond, now.wMilliseconds, log_source_name(source_file), source_line);
